...
class VideoRecorder:
    ...
    def __init__(self, 
        ...
        self.get_command = lambda path: f'ffmpeg {self.loglevel} -threads 1 -y -s {self.width}x{self.height}  -pixel_format {input_pix_fmt} -f rawvideo -r {self.fps} -i pipe: -vcodec {encoder} -pix_fmt yuv420p {path} {self.x265_params}'
        self.writer = None
        self.timestamps = []
        self.start_time = 0.0
        self.path = None
